/*
  ...
  1. New Tables
    ...
    - dispute_evidence (dispute documentation)
  
  2. Security
    - Enable RLS on all tables
    - Add policies for listings, orders, disputes
    - Add policies for dispute messages and evidence
*/
...
CREATE POLICY "dispute_messages_select_policy" ON dispute_messages
  ...
  USING (
    EXISTS (
      ...
      AND (
        disputes.created_by_id = auth.uid() OR
        EXISTS (
          SELECT 1 FROM orders
          WHERE orders.id = disputes.order_id
          AND (
